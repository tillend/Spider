...
public class BlogListPageProcessor implements PageProcessor {
	...
	private void processNewPage(Page page) {
		...
		for (Selectable s : selectable.nodes()) {
			...
			// 单个结果封装处理
			//TODO blogerId
			System.out.println("文章id：" + articleId + ",文章标题：" + linkTitle + ",文章类型:" + type
					+ ",发表日期：" + publishDateTime + ",阅读量:" + viewNum + ",评论数：" + commentNum + ",文章地址："
					+ detailUrl + ",文章摘要：" + summary + "");
		}
	...
	private void processOldPage(Page page) {
		...
		//从爬虫队列中去除当前url
		page.getTargetRequests().remove(page.getUrl());
